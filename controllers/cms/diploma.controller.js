...
class Diploma {
  ...
  async GetDiplomaById(req, res) {
    try {
      ...
      let data = await register_model
        ...
        .select(
          "id_Class id_student score_30 score_70 total_score diploma_code image_diploma"
        );
  ...
  async UploadImageDiploma(req, res) {
    try {
      let id_diploma = req.params.id;
      let body = _extend({}, req.body);

      let data = await register_model.findByIdAndUpdate(id_diploma, {
        image_diploma: body.image_list,
      });
      if (!data) return responseError(res, 400, 48, "REGISTER_LEARN_NOT_FOUND");
