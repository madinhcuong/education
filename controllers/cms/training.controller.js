...
class Training {
  ...
  async CreateTraining(req, res) {
    try {
      ...
      let data = await new Training_model(body).save();
  ...
  async UpdateStatusTraining(req, res) {
    try {
      let body = _extend({}, req.body);
      let id_training = req.params.id;

      let data_training = await Training_model.findByIdAndUpdate(id_training, {
        status: body.status,
      });
      if (!data_training)
